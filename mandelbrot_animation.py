...
def generate_mandelbrot_animation(
        ...
        start_z0, end_z0, step_z0, do_fault):
    """Yields PIL.Images representing the given mandelbrot animation."""
    if do_fault:
        logging.warning('A test fault has been requested')
        raise Exception("Alpha Beta Gamma Delta Epsilon")
...
class MandelbrotHandler(webapp2.RequestHandler):
    ...
    def get(self):
        logging.info('Somebody requested the main page')
        ...
        logging.info('The image will be built with {0} iterations'.format(iterations))
        ...
        do_fault = bool(self.request.get('do_fault'))
